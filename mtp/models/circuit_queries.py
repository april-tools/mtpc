...
from cirkit.backend.torch.layers import TorchLayer, TorchInputLayer, TorchInnerLayer, TorchHadamardLayer
...
class ArgmaxQuery(Query):
    ...
    def __call__(self) -> Tensor:
        ...
        argmax = self._circuit.evaluate(
            module_fn=functools.partial(
                self._layer_collect_argmax_fn,
                max_values=max_values
            ),
        )
        # samples: (1, O, K, D)
        argmax = argmax.permute(2, 0, 1, 3)
        # TODO: fix for the case of multi-output circuits, i.e., O != 1 or K != 1
        argmax = argmax[:, 0, 0]  # (1, D)
        return argmax

    def _layer_forward_fn(self, layer: TorchLayer, *inputs: Tensor, max_values: dict[TorchLayer, Tensor]) -> Tensor:
        # MAP from an input layer
        if not inputs:
            assert isinstance(layer, TorchBatchedCategoricalLayer)
            max_log_probs, max_vals = layer.mode()
            max_values[layer] = max_vals
            return max_log_probs

        # MAP through an inner layer
        assert isinstance(layer, TorchInnerLayer)
        if isinstance(layer, TorchBatchedSumLayer):
            max_log_probs, max_vals = layer.mode(*inputs)
            max_values[layer] = max_vals
            return max_log_probs
    ...
    def _layer_collect_argmax_fn(self, layer: TorchLayer, *inputs: Tensor, max_values: dict[TorchLayer, Tensor]) -> Tensor:
        # Argmax from an input layer
        if not inputs:
            assert isinstance(layer, TorchBatchedCategoricalLayer)
            return self._pad_argmax(max_values[layer], layer.scope_idx)

        # Argmax through an inner layer
        assert isinstance(layer, TorchInnerLayer)
        if isinstance(layer, TorchBatchedSumLayer):
            max_vals = max_values[layer]
            in_argmax, = inputs
            return layer.argmax(in_argmax, max_vals)
        elif isinstance(layer, TorchHadamardLayer):
            in_argmax, = inputs
            return torch.sum(in_argmax, dim=1)
        raise NotImplementedError()

    def _pad_argmax(self, argmax: Tensor, scope_idx: Tensor) -> Tensor:
        """Pads univariate samples to the size of the scope of the circuit (output dimension)
        according to scope for compatibility in downstream inner nodes.
        """
        if scope_idx.shape[1] != 1:
        ...
        # padded_argmax: (F, K, num_argmax, D)
        padded_argmax = torch.zeros(
            (*argmax.shape, len(self._circuit.scope)), device=argmax.device, dtype=argmax.dtype
        )
        fold_idx = torch.arange(argmax.shape[0], device=argmax.device)
        padded_argmax[fold_idx, :, :, scope_idx.squeeze(dim=1)] = argmax
        return padded_argmax
