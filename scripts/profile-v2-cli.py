...
def main():
    ...
    parser.add_argument("--legacy-lora-speculative", action="store_true", help="Use merged LoRA model instead of split LoRA/ base encoders for speculative decoding.")
    ...
    records: list[dict] = []
    for run in run_grid:
        rec = profile_run(run, device=device, prompt=args.prompt, num_tokens=args.num_tokens, use_cache=args.use_cache, draft_top_p=args.draft_top_p, target_top_p=args.target_top_p, disable_eos=args.disable_eos, legacy_lora_speculative=args.legacy_lora_speculative, base_overrides=base_overrides, tokenizer=tokenizer)
        records.append(rec)
