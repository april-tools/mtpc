...
ax = sns.scatterplot(data=df, x='acceptance_rate', y='latency', hue='Model',
...
if filename == "mtpc3-llama-argmax-nolora.csv":
    ax.set_xlim(2.5, 4.5)
...
elif filename == "mtpc3-llama-sampling-nolora.csv":
    ax.set_xlim(1.5, 2.5)
    ax.set_ylim(0.02, 0.044)
    ...
    for c in cs:
        ax.plot(x, x/c, ls='--', lw=0.5, c='black', alpha=0.5)
elif filename == "mtpc3-evabyte-sampling-nolora.csv":
    ax.set_xlim(5, 7.1)
    ax.set_ylim(0.025, 0.044)
    ...
    for c in cs:
        ax.plot(x, x/c, ls='--', lw=0.5, c='black', alpha=0.5)
elif filename == "mtpc3-evabyte-argmax-nolora.csv":
    ax.set_xlim(6.5, 9.)
    ax.set_ylim(0.025, 0.044)
    ...
    raise ValueError(f"Unexpected csv filename: {args.csv}")
...
# plt.tight_layout()
plt.savefig(f"{os.path.dirname(args.csv)}/fig3-{name}.pdf", bbox_inches="tight")
