...
# Does increasing r lead to better throughput?
##  n=4, r=1
torchrun --standalone \
    --nproc_per_node=$GPUS \
    -m mtp.train \
...
# Does using an HMM lead to better throughput?
##  n=4, r=2, hmm
torchrun --standalone \
    --nproc_per_node=$GPUS \
    -m mtp.train \
    ...
    lm.model.freeze=true \
    training.device_batch_size=$DEVICE_BATCH_SIZE \
    training.expname=basharin-hmm-ce-lora-32-n-4-r-2 \
    compile=false
...
# Does further reducing gamma lead to better throughput?
##  n=4, r=2, gamma=0.5
torchrun --standalone \
    --nproc_per_node=$GPUS \
    -m mtp.train \
    ...
    lm.model.freeze=true \
    training.device_batch_size=$DEVICE_BATCH_SIZE \
    training.expname=basharin-cp-ce-lora-32-n-4-r-2-g-50 \
    compile=false
