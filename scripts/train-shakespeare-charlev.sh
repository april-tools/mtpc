...
#          training.save_model_every=200 \
#          training.expname=shcharlev-fr-$freeze-linear-$model-n-$n_token-r-$n_component-ce-dsc
#      done
#    done
#  done
#done
    ...
    do
      ...
      do
        torchrun --standalone --nproc_per_node=$GPU -m mtp.train data=shakespeare_char training=shakespeare_char \
          ...
          lm.model.lm=null lm.model.from_checkpoint="$checkpoint" \
          training.save_model_every=200 \
          training.expname=shcharlev-fr-$freeze-transf-$model-n-$n_token-r-$n_component-ce-dsc
      done
    done
  done
done
